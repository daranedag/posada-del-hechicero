...
export default async function TournamentPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return (
    <section className="pdh-container py-12 sm:py-16">
      <div className="mb-8 rounded-[1.25rem] bg-[#142821] p-7 text-[#f6ecda] sm:p-10">
        <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-[#e6aa79]"><ShieldCheck className="size-4" /> {tournament.format_code} · Código {tournament.code}</div>
        ...
        <div className="mt-6 grid gap-3 text-sm text-white/65 sm:grid-cols-2">
          <p className="flex items-center gap-2"><CalendarClock className="size-4 text-[#e6aa79]" /> Torneo: {formatChileDate(tournament.starts_at)}</p>
          <p className="flex items-center gap-2"><MapPin className="size-4 text-[#e6aa79]" /> {tournament.location}</p>
          <p className="flex items-center gap-2 sm:col-span-2"><CalendarClock className="size-4 text-[#e6aa79]" /> Cierre de listas: {formatChileDate(tournament.submission_deadline)}</p>
        </div>
